    ...
    if (ProdExiste) {
      setListaCarrito(
        listaCarrito.map(item =>
          item.id === producto.id
            ? {...ProdExiste, cantidad: ProdExiste.cantidad + producto.cantidad}
            : item
        )
      );
    } else {
...
setListaCarrito(
    listaCarrito.filter((it) => it.id !== prodElimina.id)
)
...
const handlerStock = (contador) => {
    ...
    //console.log('Esto tiene el contador:',stockRestar)
}
